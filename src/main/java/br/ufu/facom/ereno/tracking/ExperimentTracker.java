...
public class ExperimentTracker {
    ...
    public String trackBenignDataset(String experimentId, String filePath, String configPath, 
        ...
        try {
            ...
            // Try to get attribute count from file by reading the header line directly.
            // We avoid Weka's DataSource here because the benign ARFF/CSV produced by
            // create_benign omits the SV-derived columns (they are filled in later by
            // create_attack_dataset), which can confuse Weka's loaders and produces a
            // noisy "No source has been specified" stack trace.
            entry.numAttributes = countAttributesFromHeader(filePath, format);
    ...
    /**
     * Cheaply count the number of attributes in a benign dataset by reading
     * only its header. For ARFF, this counts {@code @attribute} declarations;
     * for CSV, it counts comma-separated tokens on the first line. Returns
     * -1 if the file cannot be read.
     */
    private static int countAttributesFromHeader(String filePath, String format) {
        try (java.io.BufferedReader reader = new java.io.BufferedReader(new java.io.FileReader(filePath))) {
            if ("arff".equalsIgnoreCase(format)) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (trimmed.toLowerCase().startsWith("@attribute")) {
                        count++;
                    } else if (trimmed.equalsIgnoreCase("@data")) {
                        break;
                    }
                }
