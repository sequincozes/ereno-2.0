...
public class ARFFWritter {
    ...
    /**
     * When true, the {@code @attribute class} declaration written by
     * {@link #writeDefaultHeader()} uses the binary label set
     * ({@code normal, attack}) instead of the multiclass set. Callers must
     * set this BEFORE invoking {@link #startWriting(String)} so the header
     * matches the labels that will appear on data rows. Defaults to
     * {@code false} (multiclass) and is reset by
     * {@link #finishWriting()} to avoid leaking state across writes.
     */
    private static boolean binaryClassification = false;

    public static void setBinaryClassification(boolean binary) {
    ...
    public static void processDataset(PriorityQueue<EthernetFrame> stationBusMessages, ArrayList<EthernetFrame> processBusMessages) throws IOException {
        ...
        while (!stationBusMessages.isEmpty()) {
            ...
            if (previousGoose != null) { // skips the first message
                ...
                // Persist the 19 SV-derived columns onto the Goose (see CSVWritter for rationale).
    ...
    String classLine = "@attribute class {"
            + (binaryClassification ? Labels.asArffSetBinary() : Labels.asArffSet())
            + "}";
    ...
    public static void finishWriting() throws IOException {
        bw.close();
        // Reset to default multiclass mode so the next writer (which may
        // be invoked from a different action without explicit configuration)
        // does not inherit a stale binary header.
        binaryClassification = false;
    }

    /**
     * ARFF counterpart to
     * {@link CSVWritter#processDatasetWithAttachedSv(PriorityQueue, java.util.List)}.
     * Emits the wide 54-column dataset using SV prefixes already attached to
     * each {@link Goose}, with nearest-by-time fallback for GOOSE created by
    ...
    public static void processDatasetWithAttachedSv(PriorityQueue<EthernetFrame> stationBusMessages,
        ...
        writeDefaultHeader();
        CSVWritter.SnapshotLookup lookup = new CSVWritter.SnapshotLookup(snapshotIndex);

        Goose previousGoose = null;
        Logger.getLogger("ARFFWritter").info(stationBusMessages.size() + " mensagens na fila (snapshot mode).");
        while (!stationBusMessages.isEmpty()) {
            Goose goose = (Goose) stationBusMessages.poll();
            if (previousGoose != null) {
                ...
                String gooseString = goose.asCSVFull();
                String gooseConsistency = IntermessageCorrelation.getConsistencyFeaturesAsCSV(goose, previousGoose);
                double svTime = CSVWritter.SnapshotLookup.parseTime(svPrefix);
