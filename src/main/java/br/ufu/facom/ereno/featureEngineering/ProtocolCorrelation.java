...
public class ProtocolCorrelation {
    ...
    public static Sv getCorrespondingSVFrame(ArrayList<EthernetFrame> svs, Goose goose) {
        ...
        while (low <= high) {
            int mid = (low + high) >>> 1;
            svTime = ((Sv) svs.get(mid)).getRawTime();
        ...
        if (index >= 0) {
            // Do NOT mutate the shared SV's relativeness here. Each GOOSE
            // claiming the same SV would otherwise overwrite a previous
            // setting, making the function non-idempotent. The cycle
            // variant below was already fixed; this matches it.
            return (Sv) svs.get(index);
        } else {
    ...
    public static SVCycle getCorrespondingSVFrameCycle(ArrayList<EthernetFrame> svs, Goose goose, int numCycleMsgs) {
        ...
        // Same fix as {@link #getCorrespondingSVFrame}: search on raw,
        // monotonic SV time without mutating any sample's relativeness
        // field as a side effect.
        while (low <= high) {
            int mid = (low + high) >>> 1;
            double svTime = ((Sv) svs.get(mid)).getRawTime();
            if (svTime < gooseTimestamp) {
                index = mid;
                low = mid + 1;
            } else {
