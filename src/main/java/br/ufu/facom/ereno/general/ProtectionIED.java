...
public class ProtectionIED extends IED {
    ...
    public void addMessage(EthernetFrame periodicGoose) {
        // The legacy `ConfigLoader.gooseFlow.numberOfMessages` cap was removed:
        // it conflated the GOOSE count, the SV count, and the addMessage cap
        // into a single static and broke whenever an attack action ran in a
        // JVM that had not first executed CreateBenignAction. Each caller
        // (UC0x creators) bounds its own loop, so the cap added no safety.
        // LegitimateProtectionIED.addMessage already had the same cap
        // commented out for grayhole; this brings ProtectionIED in line.
