...
public class ProtocolCorrelation {
    ...
    public static Sv getCorrespondingSVFrame(ArrayList<EthernetFrame> svs, Goose goose) {
        if (svs == null || goose == null || svs.isEmpty()) {
            Logger.getLogger(ProtocolCorrelation.class.getName())
                    .warning("Invalid inputs to getCorrespondingSVFrame()");
            return null;
        }
        ...
        while (low <= high) {
            int mid = (low + high) / 2;
            EthernetFrame frame = svs.get(mid);
            if (!(frame instanceof Sv)) {
                // Skip non-SV frames
                // Optionally, remove or separate non-SV frames before calling this method
                mid++;
                continue;
            }

            Sv sv = (Sv) frame;
            double diff = Math.abs(sv.getTime() - gooseTimestamp);
            sv.setRelativeness((int) gooseTimestamp);
